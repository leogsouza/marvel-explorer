...
const getCharacterSearch = async (query: string, page: number = 0) => {
  try {
    ...
    console.error("error searching by character", error)
...
<template>
  <div>
    ...
    <div v-if="data && data.length > 0 && !isSearching && searchQuery">
      ...
      <Pagination :total-pages="totalPages" :path="`/search/${searchQuery}/`" :current-page="+currentPage"></Pagination>
    </div>
    <div v-if="data && data.length === 0 && searchQuery"
